            ...
            <div class="d-flex align-items-center">
              ...
              <div class="form-check mx-2">
                <input
                  ...
                  on:change="{e => onChange(index, "enabled", e.target.checked)}"
                >
                <label
                  class="form-check-label"
                  for="providers_enabled_{index}"
                >
                  Enabled
                </label>
              </div>
            ...
            {#if item.postEnabled}
              <div class="mt-2 d-flex align-items-center">
                ...
                <input
                  ...
                  on:change="{e => onChange(index, "postValue", e.target.value)}"
                >
              </div>
            {/if}
                ...
                {#each groups as group, groupIndex}
                  <div class="form-check mx-2">
                    <input
                      type="checkbox"
                      value="{groupIndex + 1}"
                      checked="{item.group & Math.pow(2, groupIndex) ? 'checked' : ''}"
                      class="form-check-input"
                      id="group_{index}_{groupIndex}"
                      disabled="{group.enabled ? '' : 'disabled'}"
                      on:change="{e => onChange(index, "group", item.group ^ Math.pow(2, groupIndex))}"
                    >
                    <label
                      class="form-check-label {group.enabled ? '' : 'text-muted'}"
