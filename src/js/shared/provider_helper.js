...
function providerTabHelper(
  ...
  var tab = {
    initialize: function () {
      return Promise.all([tab.initializeConfig(), tab.initializeQueries()]);
    },
    ...
    initializeConfig: async function () {
      ...
      document
        .querySelector(`form[name="${configForm}"] button[type="reset"]`)
        .addEventListener("click", tab.undoConfigChanges);

      // Add click/change listeners to inputs.
      _.forEach(settings, function (item) {
        var elem = document.querySelector(
          `form[name="${configForm}"] input[name="${item.key}"]`
        );
        if (item.autofiller) {
    ...
    initializeQueries: async function () {
      ...
      document
        .querySelector(`form[name="${queriesForm}"] button[type="reset"]`)
        .addEventListener("click", tab.undoQueriesChanges);
      document
        .querySelector(`form[name="${queriesForm}"] button[name="add_new"]`)
        .addEventListener("click", tab.addQuery);

      // Make list sortable.
      Sortable.create(
        document.querySelector(`form[name="${queriesForm}"] ul.list-group`),
      ...
      _.forEach(provData.queries, function (item, index) {
        document
          .querySelector(
            `form[name="${queriesForm}"] button[name="delete_${index}"]`
          )
          .addEventListener("click", tab.removeQuery);
      });

      // Add click/change listeners to inputs (except for the ones from the creation form).
      var inputs = document.querySelectorAll(
        `form[name="${queriesForm}"] input`
      );
    ...
    addQuery: async function (event) {
      ...
      var errMsg;
      if (
        _.isEmpty(formData.get("label_new")) ||
        _.isEmpty(formData.get("query_new"))
      ) {
      ...
      document.querySelector(
        `form[name="${queriesForm}"] input[name="label_new"]`
      ).value = "";
      document.querySelector(
        `form[name="${queriesForm}"] input[name="query_new"]`
      ).value = "";
